...
--> Package Information:
    --------------------
    This package enables analysis of PewDiPie's Top 20 videos and plot graphs based
...
def plotVideoViewCountGraph(infosView):
    try:
        ...
        plt.title("Top 20 most viewed videos of PewDiPie")
        plt.setp(values.get_xticklabels(), rotation=30, horizontalalignment='right')
        plt.tight_layout()
        plt.savefig('./data/graphical_analysis_pewdipie/pewDiPie_most_viewed.png')
        plt.show()
    except Exception as e:
        print("The exception occurred is: \t{0}".format(e))
...
def plotVideoCommentCountGraph(infosComment):
    try:
        ...
        plt.title("Top 20 most commented videos of PewDiPie")
        plt.setp(values.get_xticklabels(), rotation=30, horizontalalignment='right')
        plt.tight_layout()
        plt.savefig('./data/graphical_analysis_pewdipie/pewDiPie_most_commented.png')
        plt.show()
    except Exception as e:
        print("The exception occurred is: \t{0}".format(e))
...
def plotVideoLikesCountGraph(infosLikes):
    try:
        ...
        plt.title("Top 20 most liked videos of PewDiPie")
        plt.setp(values.get_xticklabels(), rotation=30, horizontalalignment='right')
        plt.tight_layout()
        plt.savefig('./data/graphical_analysis_pewdipie/pewDiPie_most_liked.png')
        plt.show()
    except Exception as e:
        print("The exception occurred is: \t{0}".format(e))
...
def plotVideoDislikesCountGraph(infosDislikes):
    try:
        ...
        plt.title("Top 20 most disliked videos of PewDiPie")
        plt.setp(values.get_xticklabels(), rotation=30, horizontalalignment='right')
        plt.tight_layout()
        plt.savefig('./data/graphical_analysis_pewdipie/pewDiPie_most_disliked.png')
        plt.show()
    except Exception as e:
        print("The exception occurred is: \t{0}".format(e))
...
commentDataJson = pd.DataFrame.from_dict(commentThreads).to_json("./json_files/commentsJSON_PewTop20V.json", orient="table")
commentDataCsv = pd.DataFrame.from_dict(commentThreads).to_csv("./csv_files/commentsCSV_PewTop20V.csv", encoding='utf-8')

#------------< Getting top 20 most used words in the comments of videos retrieved >------------
def getMostUsedWords():
    try:
        all = pd.read_csv("./csv_files/commentsCSV_PewTop20V.csv")
...
def plotMostUsedWordsGraph():
    try:
        ...
        plt.tight_layout()
        plt.savefig('./data/graphical_analysis_pewdipie/wordFrequency.png')
        fig.show()
    except Exception as e:
        print("The exception occurred is: \t{0}".format(e))
...
def trainingClassifierNaiveBayes():
    try:
        ...
        for video in videos:
            ...
            plt.savefig('./data/sentiment_analysis_pewdipie/pewDiPie_'+noSpaceVideoName+'.png')
            plt.show()
    except Exception as e:
        print("The exception occurred is: \t{0}".format(e))
